...
const AddAssociado = () => {
  const datalocation = sessionStorage.getItem('rowDataAssociado');  
  const history = useHistory();
  const { message } = useSnackbar();  
  const Title = "Cadastro de Associado";
  ...
  const handleSubmit = async () => { 
    ...
    {
      result = await api(`${ApiAssociado}/${values.id}`, 'PUT', JSON.stringify(values));
    }
    ...
    if (result.error) {
      ...
      wait(2000).then(() => { history.push("/Associados"); });
    } 
  };    
  ...
  return (    
    <div style={{ maxWidth: 1130, margin: "0 auto", paddingTop: 32, position: "relative" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <Breadcrumbs
          items={[
            ...
            { title: "Associados", as: Link, props: { to: "/Associados" }, icon: Person },
            { title: Title }
          ]}
        />
      </div>
      ...
      <div style={{ marginTop: 24, display: "flex", justifyContent: "flex-end" }}>
        ...
        <Link style={{ textDecoration: "none" }} to="/Associados">
