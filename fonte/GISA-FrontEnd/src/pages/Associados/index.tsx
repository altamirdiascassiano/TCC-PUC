...
import { ApiAssociado } from "../../services/base"
...
const Associados = () => {  
  ...
  async function fetchAssociados() {
    ...
    const result = await api(`${ApiAssociado}`, 'GET');
    if (result.error) {
  ...
  const handleCreate = () => {    
    sessionStorage.clear();
    history.push("/Associados/Associado");
  }

  const handleDelete = async (rowData: any) => {
    setIsFetching(true);
    const result = await api(`${ApiAssociado}/${rowData.id}`, 'DELETE');
    if (result.error) {
      console.log(result.error) // eslint-disable-line no-console
  ...
  const handleEdit = (rowData: any) => {
    sessionStorage.setItem("rowDataAssociado", JSON.stringify(rowData));
    history.push("/Associados/Associado");
  }
  ...
  return (    
    <div style={{ maxWidth: 1500, margin: "0 auto", paddingTop: 32, position: "relative" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <h3 style={{ margin: 0, color: "red" }}>Associados do Boa Saúde</h3>
        <Breadcrumbs
          items={[
            { title: "Home", as: Link, props: { to: "/home" }, icon: Home },
            { title: "Associados", icon: Person }
          ]}
        />
      </div>
      ...
      <Paper elevation={10} style={{ marginTop: 24 }}>        
        <MaterialTable
          ...
          columns={[
            ...
            { title: 'CPF', field: 'cpf', searchable: true},
          ...
          options={{
            ...
            grouping: false,   
            headerStyle: { position: 'static', top: 0, backgroundColor: "#c0d0d0"}, maxBodyHeight: '3000px',
            selection: false
          }}
        />  
      </Paper>
